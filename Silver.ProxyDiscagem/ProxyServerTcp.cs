...
namespace Silver.ProxyDiscagem
{
    public class ProxyServerTcp
    {
        ...
        private int porta_conexao = Convert.ToInt32(ConfigurationManager.AppSettings["application.tcpserver.port"]);
        ...
        public void AguardarConexao()
        {
            while (em_execucao)
            {
                ...
                ClienteConectado cliente_conectado = new ClienteConectado(BeginNovaConexao);
                cliente_conectado.BeginInvoke(novo_cliente, new AsyncCallback(EndNovaConexao), ProtocoloConexao.ToString("000000"));
            }
        }
