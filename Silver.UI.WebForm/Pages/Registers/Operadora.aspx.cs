...
{
    public partial class Operadora : PaginaBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            Master.NomeAplicacao = "Operadora";
            Master.DescricaoAplicacao = "Configuração e manutenção de Operadora";

            if (!IsPostBack)
            {
                tbConsumoMinino.Attributes.Add("onkeypress", "if (event.keyCode < 48 || event.keyCode > 58) {event.keyCode = 0;}");
        ...
        protected void GridViewObjeto_Preencher(int intPageIndex, string filtro = "")
        {
            ...
            var Operadora = new BLL.Operadora();

            if (string.IsNullOrEmpty(filtro))
                GridViewObjeto.DataSource = Operadora.Listar();
            else
                GridViewObjeto.DataSource = Operadora.Buscar(filtro);
        ...
        {
            var Operadora = new BLL.Operadora();
            GridViewObjeto.DataSource = Operadora.Buscar(tbBuscar.Text);
            GridViewObjeto.DataBind();
        }
        ...
        {
            LimparCampos();
            var Operadora = new BLL.Operadora().Obter(intID);
            IdCadastrar.Value = Operadora.Id.ToString();
            tbDescricao.Text = Operadora.Descricao.ToString();
            tbConsumoMinino.Text = Operadora.ConsumoMinino.ToString();
            if (Operadora.Ativo)
            {
        ...
        protected void LimparCampos()
        {
            ...
            tbConsumoMinino.Text = string.Empty;
            cbAtivo.Checked = false;
        }

        protected void btCadastrar_Sim_Click(object sender, EventArgs e)
        {
            var DTOCadastrar = new DTO.Operadora();
            var BLLCadastrar = new BLL.Operadora();
            ...
            DTOCadastrar.ConsumoMinino = Convert.ToInt64(tbConsumoMinino.Text);
        ...
        {
            var Operadora = new BLL.Operadora();
            Operadora.Ativar(int.Parse(IdExcluir.Value), false);
