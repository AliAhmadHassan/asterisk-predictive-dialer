...
{
    public partial class TarifaTipo : PaginaBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            Master.NomeAplicacao = "Tarifa Tipo";
            Master.DescricaoAplicacao = "Configuração e manutenção de Tarifa Tipo";
        ...
        protected void GridViewObjeto_Preencher(int intPageIndex, string filtro = "")
        {
            ...
            var TarifaTipo = new BLL.TarifaTipo();
            if (string.IsNullOrEmpty(filtro))
                GridViewObjeto.DataSource = TarifaTipo.Listar();
            else
                GridViewObjeto.DataSource = TarifaTipo.Buscar(filtro);
        ...
        {
            var TarifaTipo = new BLL.TarifaTipo();
            GridViewObjeto.DataSource = TarifaTipo.Buscar(tbBuscar.Text);
            GridViewObjeto.DataBind();
        }
        ...
        {
            LimparCampos();
            var TarifaTipo = new BLL.TarifaTipo().Obter(intID);
            IdCadastrar.Value = TarifaTipo.Id.ToString();
            tbDescricao.Text = TarifaTipo.Descricao.ToString();
            if (TarifaTipo.Ativo)
            {
        ...
        {
            var DTOCadastrar = new DTO.TarifaTipo();
            var BLLCadastrar = new BLL.TarifaTipo();
        ...
        {
            var TarifaTipo = new BLL.TarifaTipo();
            TarifaTipo.Ativar(int.Parse(IdExcluir.Value), false);
        ...
        protected void GridViewObjeto_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.Cells[4].Text.Equals("False"))
            {
