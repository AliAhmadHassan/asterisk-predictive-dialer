...
{
    public partial class Rota : PaginaBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            Master.NomeAplicacao = "Rota";
            Master.DescricaoAplicacao = "Configuração e manutenção de rotas";

            if (!IsPostBack)
            {
                tbPrioridade.Attributes.Add("onkeypress", "if (event.keyCode < 48 || event.keyCode > 58) {event.keyCode = 0;}");
        ...
        protected void GridViewObjeto_Preencher(int intPageIndex, string filtro = "")
        {
            ...
            var Rota = new BLL.Rota();

            if (string.IsNullOrEmpty(filtro))
                GridViewObjeto.DataSource = Rota.Listar();
            else
                GridViewObjeto.DataSource = Rota.Buscar(filtro);
        ...
        {
            var Rota = new BLL.Rota();
            GridViewObjeto.DataSource = Rota.Buscar(tbBuscar.Text);
            GridViewObjeto.DataBind();
        }
        ...
        {
            LimparCampos();
            var Rota = new BLL.Rota().Obter(intID);
            IdCadastrar.Value = Rota.Id.ToString();
            tbDescricao.Text = Rota.Descricao;
            PopularCampos(int.Parse(Rota.IdOperadora.ToString()), int.Parse(Rota.IdTarifaTipo.ToString()));
            tbPrioridade.Text = Rota.Prioridade.ToString();
        ...
        protected void LimparCampos()
        {
            ...
            tbPrioridade.Text = string.Empty;
            cbAtivo.Checked = false;
        }

        protected void PopularCampos(int IdOperadora, int IdTarifaTipo)
        {
        ...
        {
            var DTOCadastrar = new DTO.Rota();
            var BLLCadastrar = new BLL.Rota();
            ...
            DTOCadastrar.Prioridade = Convert.ToInt64(tbPrioridade.Text);
        ...
        {
            var Rota = new BLL.Rota();
            Rota.Ativar(int.Parse(IdExcluir.Value), false);
        ...
        protected void GridViewObjeto_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            e.Row.Cells[4].Text = new BLL.Operadora().Obter(e.Row.Cells[4].Text.ToInt32()).Descricao;
            e.Row.Cells[5].Text = new BLL.TarifaTipo().Obter(e.Row.Cells[5].Text.ToInt32()).Descricao;
        }
