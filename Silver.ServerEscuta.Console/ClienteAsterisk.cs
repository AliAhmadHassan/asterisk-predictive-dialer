...
namespace Silver.ServerEscuta.Console
{
    public class ClienteAsterisk : IDisposable
    {
        ...
        {
            #region Asterisk
            SaidaLogs("Iniciando conexão com servidor Asterisk*", LoggerType.INFO);
        ...
        private void readStream(NetworkStream stream)
        {
            ...
            SaidaLogs("Conexão com servidor Asterisk* estabelecida com sucesso.", LoggerType.INFO);
            SaidaLogs("Escutando servidor Asterisk*.", LoggerType.INFO);
            ...
            while (true)
            {
                ...
                {
                    SaidaLogs(e.Message + Environment.NewLine + "NOTA DO DESENVOLVEDOR: Verificar a necessidade de restaura o status do socket", LoggerType.ERRO);
                    IniciarEscutaAsterisk();
                }
                finally
                {
                ...
                if (errorCount == 10)
                {
                    SaidaLogs("A quantidade de tentativas de conexão com o servidor Asterisk excedeu a 10x", LoggerType.ERRO);
                    return;
                }
        ...
        private void SaidaLogs(string mensagem, Silver.Logger.LoggerType tipolog)
        {
            ...
            Silver.Logger.LoggerAsync.pathLog = path_logs;
            Silver.Logger.LoggerAsync.WriteLog(new MensagemLogger { MessageLogger = mensagem, TypeLogger = tipolog });
        }
        ...
        private void RamalAtendeu(string[] Linhas)
        {
            try
            {
                SaidaLogs("O evento RamalAtedeu foi acionado.", LoggerType.INFO);
                ...
                SaidaLogs(string.Format("O ramal {0} atendeu", Ramal), LoggerType.INFO);

                string telefone = CallerID1;
                SaidaLogs("Número do cliente: " + telefone, LoggerType.INFO);
                DTO.Carga carga = new Silver.BLL.Carga().SelectPeloTelefone(telefone.Substring(telefone.Length - 8));
                ...
                if (CPF == "")
                {
                    SaidaLogs("O número do cpf está vazio.", LoggerType.INFO);
                    SaidaLogs(string.Format("O ramal {0} desligou", Ramal), LoggerType.INFO);
                    return;
                }

                var url = string.Format(ConfigurationManager.AppSettings["application.url.crm"], CPF);
                SaidaLogs("Url Informada: " + url, LoggerType.INFO);
        ...
        private void SendMessageToAsterisk(String message)
        {
            ...
            {
                SaidaLogs(e.Message, LoggerType.ERRO);
            }
        }
