...
using Silver.ProxyDiscagem.Teste.DTO;
...
namespace Silver.ProxyDiscagem.Teste
{
    ...
    public class ProxyDiscagem
    {
        ...
        /// </summary>
        private List<Channels> GruposCanaisLivres;
        ...
        private void Discar()
        {
            try
            {
                while (FilaDeSolicitacoes.Count > 0)
                {
                    ...
                    List<Rota> prioridadesRota = DefineRotas(solicitacaoDiscagem);
                    ...
                    foreach (Silver.DTO.Rota prioridade in prioridadesRota)
                    {
                        ...
                        {
                            MensagemLog(string.Format("Sem canais disponíveis no grupo: {0}", grupo.Grupo), LoggerType.INFO, ConsoleColor.Red);
                            continue;
                        }
                        else
                        {
                            protocolo = auxprotocolo;
                            MensagemLog(string.Format("DDD: {0}\tTelefone: {1}\tProtocolo: {2}\tCanais Livres: {3}", solicitacaoDiscagem.DDD, solicitacaoDiscagem.Telefone, auxprotocolo, grupo.Quantidade), LoggerType.INFO, ConsoleColor.Green);
                        }
                    }
                    ...
                    else
                    {
                        ...
                        using (StreamWriter stream_writer = new StreamWriter(solicitacaoDiscagem.Cliente.GetStream(), Encoding.ASCII))
                        {
                            ...
                            //Console.WriteLine("[{0}] - {1}", DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss.ffff"), "Verificação da fila: Nenhum canal disponível.");
            ...
            catch (Exception ex) { MensagemLog("Discar() - Falha no processamento das Discagem. A mensagem do sistema foi: " + ex.Message, LoggerType.ERRO, ConsoleColor.Red); }
            finally
            {
                processando_solicitacoes = false;
                MensagemLog("Requisições processadas! Aguardando novas Solicitações de Discagem.", LoggerType.INFO, ConsoleColor.Yellow);
            }
        }
        ...
        private void ConfigurarEscutaAsterisk()
        {
            ...
            asterisk_escuta_dgv.OnDGVShowChannels = (string valores) =>
            {
                CanaisDGV.Clear();
                ...
                foreach (string linha in valores.Split('\n'))
                {
                    ...
                    RelChannels channel = new Silver.DTO.RelChannels();
        ...
        private void MensagemLog(string mensagem, LoggerType tipo_log = LoggerType.INFO, ConsoleColor color = ConsoleColor.White)
        {
            ...
            LoggerAsync.WriteLog(new  Silver.Logger.Logger.LoggerMessage { MessageLogger = mensagem, TypeLogger = tipo_log }, color);
        }
